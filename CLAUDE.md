...
> These rules govern Claude Code's behavior in this repository. They mirror the rules in `/.github/copilot-instructions.md` (GitHub Copilot) and `/.cursorrules` (Cursor). All AI agents operating in this project follow the same governance framework.

---

## Rule 0: First-Contact Briefing (Mandatory)

When an AI agent is **first invoked** in a project that uses this template, it **MUST NOT** begin writing code, generating files, or making suggestions immediately.
...
2. **The Development Hierarchy is mandatory.** Every piece of work follows this sequence with no exceptions:
   ...
   > Items in `[brackets]` are conditional — see the Product Document Guidebook (`/governance-docs/product-doc-guidebook.md`) for when each applies. The **core mandatory sequence** is: BRD → PRD → TAD → Feature Spec → ADR → Implementation → Testing → Deployment.

   **Document merge flexibility** (per the Product Document Guidebook):
   - **BRD may merge into PRD** as a "Business Rules & Constraints" section for small teams or non-regulated products. Standalone BRDs are required for regulated products (fintech, health, insurance)
   - **API Spec starts inside the TAD** at early stage; graduates to a standalone document once external consumers exist
   - **SOW** (Statement of Work) is required only when external vendors, agencies, or contractors are involved — it is a **legal contract** and must never merge with product or technical documents
   - **FRD and User Stories are sections inside the PRD** — they are never separate documents (see PRD template)
...
4. **Document locations are fixed:**
   ...
   - All project documents (Vision Doc, BRD, PRD, SOW, TAD, GTM Plan, and any other project-level docs except ADRs and Feature Specs): `/dev-project-docs/`
...
5. **Why this exists:** These governance standards ensure consistency, auditability, security (OWASP, ISO 27001), and that decisions survive the people who made them. Every significant decision must be written down before code is written.

### Step 2 - Audit Required Documents

After the briefing, the agent must check for the existence and completeness of these critical documents:
...
| BRD (Business Requirements Document) | `/dev-project-docs/BRD-<Project-Name>.md`        | PRD can be written                | Yes (may merge into PRD for small/non-regulated teams) |
| PRD (Product Requirements Document)  | `/dev-project-docs/PRD-<Project-Name>.md`        | TAD can be written                | **Yes** |
| SOW (Statement of Work)              | `/dev-project-docs/SOW-<Project-Name>.md`        | Vendor engagement can begin       | Only when vendors are involved |
...
**If the BRD, PRD, or TAD / Solution Architecture Document is missing or empty, the agent MUST:**

1. **Stop and inform the developer clearly.** State which documents are missing, **where they should be placed**, and **what they should be named:**
...
3. **Ask the developer explicitly:**
...
4. **Do not silently skip this.** Do not proceed to implementation. Do not generate code. The hierarchy is not optional.

### Step 3 - Enter Deep Thinking Mode for Document Creation

**MANDATORY:** If the developer asks the AI agent to assist with creating governance documents (Vision Doc, BRD, PRD, TAD / Solution Architecture, Feature Specs, or ADRs), the agent **MUST activate its deepest reasoning mode before generating any content.**

- **Claude (Claude Code / API):** Use extended thinking / deep analysis. Reason through the full context of the project, the governance requirements, the templates, and the downstream impact before producing output.
- **GitHub Copilot / Cursor / Other agents:** Use the most thorough reasoning or planning mode available. Do not use autocomplete or fast-generation modes for governance documents.

**Why deep thinking is required:** These documents define architectural decisions, security boundaries, data flows, and compliance posture. A shallow or templated response undermines the entire governance framework. The agent must reason about:
...
- Whether the content aligns with the SDGP standards in `/governance-docs/sdgp-main.md`
...
- Generate implementation code for a feature that has no approved feature spec
- Write an ADR without a parent feature spec
- Skip the BRD → PRD → TAD → Spec → ADR → Implementation sequence
- Create a feature spec if no PRD or TAD / Solution Architecture Document exists (flag it instead)

If a developer asks the agent to skip the hierarchy (e.g., "just write the code"), the agent must:
...
5. Log the deviation per Rule 8 below
...
1. **Validate branch context before writing code.** Before generating implementation code, check the current branch name. If the developer is on `main`, `staging`, `production`, or any protected branch, **stop and instruct them to create a feature branch:**
   > "You are currently on `main`. All implementation work must happen on a feature branch. Shall I create `feature/<ticket>-<description>` for you?"
...
When assisting with feature development, the agent must flag when an ADR is required. An ADR is **mandatory** when any of the following is true:
...
The agent must proactively identify these triggers and remind the developer that an ADR is needed before implementation.

---

## Rule 4: Templates Are Not Optional

When creating Feature Specs, use the template at `/docs/specs/_template.md`.
When creating ADRs, use the template at `/docs/adr/_template.md`.
When creating the Solution Architecture Document, it must contain schemas as specified in the governance kit.

Do not invent alternative formats. The templates exist for consistency and auditability.

---

## Rule 5: Changelog and Versioning Discipline (Agent-Managed)

All meaningful changes to the project must be recorded in `/CHANGELOG.md` following the [Keep a Changelog](https://keepachangelog.com/) format. All releases must follow [Semantic Versioning 2.0.0](https://semver.org/) as defined in `/governance-docs/coding-standards.md` §6.3.
...
When an AI agent creates or modifies governance documents, feature specs, ADRs, or significant code, it must note its involvement. Use the following convention:

- In commits: include `Co-Authored-By` with the agent name
- In PRs: note in the Agent Review Summary section per the PR template in the governance kit
...
**Why:** Developers switch between AI tools. A developer using Copilot may have documented notes, instructions, or context in `/.github/copilot-instructions.md` that are relevant to your work. Similarly, notes may exist in `/.cursorrules`.
...
FIRST INVOCATION:
  ...
  2. Audit: PRD exists? TAD / Solution Architecture exists? Feature specs exist?
  3. If missing → inform developer → ask: self-create or need help?
  4. If assisting with doc creation → ENTER DEEP THINKING MODE
  5. Populate Project-Specific Notes when info is available (ask developer first)
  6. Only then proceed to requested work

EVERY SESSION:
  - Review /.github/copilot-instructions.md and /.cursorrules for developer notes
  - Flag anything relevant or conflicting

ALWAYS:
  - Respect [Vision] → BRD → PRD → [SOW] → TAD → Spec → ADR → Code hierarchy
